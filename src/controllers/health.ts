...
import { ConfigProvider, AppConfig, AppConfigMaskedList } from '../config';

export class HealthModule implements ExpressRequestHandler {
  ...
  requestHandler(
    ...
    response.send(
      JSON.stringify({
        status: 'OK',
        time: localTime,
        sha: this.config.get(AppConfig.sha),
